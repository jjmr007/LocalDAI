/**
 *Originally Submitted for verification at Etherscan.io on 2018-06-03 for local ethereum exchange
 * Rewrite to trade with new DAI contract: 0x6B175474E89094C44Da98b954EedeAC495271d0F
*/
...
/// @title clubdeletras.com
/// @author clubdeletras.com
contract LocalDaiEscrows {
    /***********************
    +   Global settings   +
    ***********************/
    ...
    address public DAI;
    // Address of the arbitrator (currently always clubdeletras staff)
    ...
    ***********************/
    ...
    /// @param _tradeID The unique ID of the trade, AUTOMATICALLY GENERATED BY clubdeletras.com
    /// @param _seller The selling party
    /// @param _buyer The buying party
    /// @param _value is The amount of the escrow, exclusive of the fee
    /// @param _fee CLUBDELETRAS's commission in 1/10000ths or basis points (bps)
    /// @param _params time related: [0] for _paymentWindowInSeconds, [1] for _expiry
    /// @param _v relayer's Signature "v" component
    /// @param _V0 Seller's Signature "v" component
    /// @param _sigs Relayer's (R = _sigs[0] and S= _sigs[1]) and Seller's (R = _sigs[2] and S = _sigs[3]) Signature components "r" and "s"
    ...
    function createEscrow(
        ...
        uint256 _value,
        uint16 _fee,
        uint32[2] memory _params,
        // _params[0] = _paymentWindowInSeconds, The time in seconds from escrow creation that the seller can cancel after
        // _params[1] = _expiry, This transaction must be created before this time
        uint8 _v,
        uint8 _V0,
        bytes32[4] memory _sigs

    )
    // payable // this feature (payability) is meant to be cancelled
    public {
        ...
        // A signature (v, _r and _s) must come from localethereum to open an escrow
        bytes32 _invitationHash = keccak256(abi.encodePacked(
            _tradeHash,
            _params[0],
            _params[1]
        ));
        require(recoverAddress(_invitationHash, _v, _sigs[0], _sigs[1]) == relayer, "Must be relayer");
        // These signatures come with an expiry stamp
        require(block.timestamp < _params[1], "Signature has expired");

        /*
        * here goes the crucial change
        * seller transfer to the contract DAI tokens
        * instead of ether
        */

        // Before all, let retrieve the DAI's contract nonce of the _seller

        (bool success, bytes memory data) =
        DAI.call(abi.encodeWithSelector(0x7ecebe00 /* nonces */, _seller));
        require(success, "nonce call error");
        uint256    nonce;
        // retreive the data
        if (data.length > 0) {
            require(data.length == 32, "data extension must be 0 or 32 bytes");
            nonce = abi.decode(data, (uint));
            nonce++;
            }

        // FIRST step, modify 'allow':

        (success, data) =
        DAI.call(abi.encodeWithSelector(0x80c33b34 /* permit */, _seller, address(this), nonce, 0 /* do not allow to expire */ , true /* the boolean "allowed" */, _V0, _sigs[2], _sigs[3]));
        require(success, "permit signature error");

        // check the data received is `true`
        if (data.length > 0) {
            require(data.length == 32, "data extension must be 0 or 32 bytes");
            success = abi.decode(data, (bool));
            require(success, "call error. DAI permit resolving false");
            }

        // SECOND step, send the tokens to the escrow contract:

        (success, data) = DAI.call(abi.encodeWithSelector(0x23b872dd /* transferFrom */,  _seller, address(this), _value + uint256(_fee)));
        require(success, "insuficient balance");

        // check the data received is `true`
        if (data.length > 0) {
        require(data.length == 32, "data extension must be 0 or 32 bytes");
        success = abi.decode(data, (bool));
        require(success, "call error. DAI transferFrom resolving false");
            }
        ...
        require(_value > 0, "Incorrect DAI sent");

        uint32 _sellerCanCancelAfter = _params[0] == 0
            ? 1
            : uint32(block.timestamp) + _params[0];
    ...
    function getRelayedSender(
      ...
      bytes32 _s
        ...
        require(_maximumGasPrice >= tx.gasprice);
        /*
